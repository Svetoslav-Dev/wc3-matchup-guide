...
export function NewBuildingForm({ races }: Props) {
  ...
  const { imageError, imageLimitLabel, onImageChange, onSubmit } = useImageUploadValidation("Buildings");
  ...
  return (
    <>
      ...
      {imageError ? <div className="status-banner status-banner--error" style={{ maxWidth: "560px", margin: "0 auto", width: "100%" }}>{imageError}</div> : null}
      <form key={state?.key ?? 0} action={formAction} style={{ display: "flex", justifyContent: "center" }} onSubmit={onSubmit}>
